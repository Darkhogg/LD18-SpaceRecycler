...
public final class TriangleEnemy extends Enemy {
	...
	public TriangleEnemy ( Game game, double x, double y ) {
		...
		double ang = Math.atan2( game.player.posX-posX,
	...
	public Collection<Bullet> getBullets () {
		if ( ticks-lastShoot >= Game.FPS*1.5 ) {
			...
			for ( int i=0; i<3; i++ ) {
				Bullet bul = new EnemyBullet( game, posX, posY,
					spd*Math.sin( ang + i*(Math.PI*2/3) ) + spdX,
					-spd*Math.cos( ang + i*(Math.PI*2/3) ) + spdY );
				shoots.add( bul );
			}
